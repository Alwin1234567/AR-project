...
def berekeningen_init(sheet, deelnemer, logger):
    ...
    for i, flexibilisatie in enumerate(deelnemer.flexibilisaties):
        ...
        blok.append(["OP''H / OP''L", '=IF(B{0} =  "", B{2}, IF(B{0} = "Verhouding",  ROUND((B{2} * B{3}) / IF(C{0} = "HL", B{4} + C{1} * B{5}, C{1} * B{4} + B{5}), 2), ROUND(B{2} + IF(C{0} = "HL", C{1} * B{4}, C{1} * B{5}) / B{3}, 0)))'.format(blokhoogte + 4, blokhoogte + 5, blokhoogte + 9, blokhoogte + 12, blokhoogte + 15, blokhoogte + 16),\
